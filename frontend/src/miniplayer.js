...
setTimeout(() => { audio.addEventListener('volumechange', () => { updateMiniPlayer(); }); }, 0);
...
export async function toggleMiniPlayer() {
  ...
  invoke('mini_toggle').catch(() => {});  // ouvre (ou ferme) la fenêtre mini flottante
  invoke('win_minimize').catch(() => {}); // réduit la fenêtre principale
}
